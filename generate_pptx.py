...
def add_dashboard_focus(slide, img_path, top):
    """Show the dashboard header at a readable scale, then summarize all 11 patterns."""
    from PIL import Image

    image = Image.open(img_path)
    # Crop by ratio (the capture is hi-DPI) so no pattern section appears half cut off.
    focus = image.crop((0, 0, image.width, int(image.height * 0.18)))
    buffer = BytesIO()
    focus.save(buffer, format="PNG")
    buffer.seek(0)

    panel_w = Inches(11.8)
    panel_h = Inches(4.15)
    img_w, img_h = focus.size
    scale = min(panel_w / img_w, panel_h / img_h)
    final_w = int(img_w * scale)
    final_h = int(img_h * scale)
    left = int((SLIDE_W - final_w) / 2)
    slide.shapes.add_picture(buffer, left, int(top + (panel_h - final_h) / 2), final_w, final_h)
    ...
    for panel_left, heading, detail, color in categories:
        add_textbox(slide, panel_left, Inches(5.75), Inches(3.88), Inches(0.32),
                    heading, font_size=16, color=color, bold=True, alignment=PP_ALIGN.CENTER)
        add_textbox(slide, panel_left, Inches(6.18), Inches(3.88), Inches(0.32),
                    detail, font_size=10, color=TEXT_COLOR, alignment=PP_ALIGN.CENTER)
