...
A: Two frameworks are supported via separate branches: **Spring Boot** (on the `main` branch) and **Quarkus** (on the `quarkus` branch). Both are fully functional and interchangeable.
...
A: Start the backend with `mvn spring-boot:run` (or `mvn quarkus:dev` on the Quarkus branch), then start the frontend with `cd frontend && npm install && npm run dev`.
...
A: On the Spring Boot branch (`main`), the WebSocket uses **STOMP over SockJS** at endpoint `/ws`. On the Quarkus branch, it uses native Quarkus WebSockets.
